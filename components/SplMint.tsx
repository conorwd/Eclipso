// components/SplMint.tsx

import React, { useState, useCallback } from 'react';
import { useConnection, useWallet } from '@solana/wallet-adapter-react';
import { Keypair, Transaction, PublicKey } from '@solana/web3.js';
import { createSplTokenMint, SplMintParams, SplTokenMetadata, createSplMintTokensTransaction } from '../utils/SplTokenMintingUtils';
import { toast } from 'sonner';

const SplMint: React.FC = () => {
  ...
  const handleMint = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mintParams: SplMintParams = {
        connection,
      ...
      const transaction = await createSplTokenMint(mintParams);
      ...
      toast.success(`SPL Token minted successfully! Signature: ${signature}`);
    } catch (error) {
      console.error('Error minting SPL token:', error);
      if (error instanceof Error) {
        toast.error(`Error minting SPL token: ${error.message}`);
      } else {
        toast.error('An unknown error occurred while minting the SPL token');
      }
    } finally {
      setLoading(false);
    }
  }, [connection, wallet, tokenName, tokenSymbol, decimals]);
  ...
  const handleMintTokens = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const transaction = await createSplMintTokensTransaction({
        connection,
        payer: wallet.publicKey,
        mint: mintPubkey,
      ...
      toast.success(`SPL Tokens minted successfully! Signature: ${signature}`);
    } catch (error) {
      console.error('Error minting SPL tokens:', error);
      if (error instanceof Error) {
        toast.error(`Error minting SPL tokens: ${error.message}`);
      } else {
        toast.error('An unknown error occurred while minting SPL tokens');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-full max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-xl overflow-hidden">
      <div className="p-4 bg-gray-700">
        <h2 className="text-xl font-semibold text-white">
          {!mintAddress ? 'Create SPL Token' : 'Mint SPL Tokens'}
        </h2>
      </div>
      <div className="p-4">
        {!mintAddress ? (
          <form onSubmit={handleMint} className="space-y-4">
            ...
            >
              {loading ? 'Creating SPL Token...' : 'Create SPL Token'}
            </button>
          </form>
        ) : (
          <div className="space-y-4">
            <div>
              <h3 className="text-lg font-medium text-white">SPL Token Info</h3>
            ...
            <form onSubmit={handleMintTokens} className="space-y-4">
              ...
              >
                {loading ? 'Minting SPL Tokens...' : 'Mint SPL Tokens'}
              </button>
            </form>
          </div>
        )}
